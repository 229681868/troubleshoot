...
发现多架构支持被设置了ARM，然而我们的平台是X86架构的，不需要ARM平台的支持，先前的问题可能就是这个原因导致的，于是尝试删除这个设置
sudo dpkg --remove-architecture arm64
但是提示dpkg：错误：无法移除体系结构 arm64 ，当前它仍被数据库使用
最后通过多次尝试终于找到了如下解决办法，终端输入如下命令:
sudo rm -rf /var/lib/apt/lists/*
sudo apt-get remove .*:arm64
sudo dpkg --remove-architecture arm64
之后执行sudo apt-get update 即可正常使用
"""
import time
import os
import sys
...
class PreSetup(object):
    ...
    def _depend_rpm(self):
        ...
        if self._is_ubuntu:
            cmd = "sudo apt-get -y --force-yes install fio python-pip"
            ret_code = bash(cmd)
            self.check_bash_return(ret_code, "安装依赖失败, 请检查安装源是否更新或手动安装！")
...
class FioTest(object):
    ...
    def exprCmd(self):
        cmd = "fio --name=" + self.name + " --rw=" + self.rw + " --iodepth=" + str(
            self.iodepth) + " --ioengine=" + self.ioengine + " --thread --direct=" + str(
            self.direct) + " --norandommap --bs=" + self.bs + " --size=" + self.size + " --runtime=" + str(
            self.runtime) + " --filename=" + self.filename + " --minimal --output-format=json"
        # print(cmd)
        return cmd
...
if __name__ == '__main__':
    ...
    cmd1.saveResult()
    cmd2 = FioTest(name="randwrite-4k", rw="randwrite", iodepth=1, ioengine="libaio", direct=1, bs="4k", size=size4k,
                   runtime=60, filename=filename)
    cmd2.saveResult()
    cmd3 = FioTest(name="randread-4k-64", rw="randread", iodepth=64, ioengine="libaio", direct=1, bs="4k", size=size4k,
                   runtime=60, filename=filename)
    cmd3.saveResult()
    cmd4 = FioTest(name="randwrite-4k-64", rw="randwrite", iodepth=64, ioengine="libaio", direct=1, bs="4k",
                   size=size4k,
                   runtime=60, filename=filename)
    cmd4.saveResult()
    cmd5 = FioTest(name="seq-read-128k", rw="read", iodepth=64, ioengine="libaio", direct=1, bs="128k", size=size128k,
                   runtime=60, filename=filename)
    cmd5.saveResult()
    cmd6 = FioTest(name="seq-write-128k", rw="write", iodepth=64, ioengine="libaio", direct=1, bs="128k", size=size128k,
                   runtime=60, filename=filename)
    cmd6.saveResult()
    printResult()
